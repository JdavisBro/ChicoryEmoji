    ...
    foreach (Node n in atlas.Nodes)
    {
        if (n.Texture != null)
        {
            ...
            else if (spriteType == SpriteType.Sprite)
            {
                ...
                try
                {
                    spriteName = new DirectoryInfo(Path.GetDirectoryName(n.Texture.Source)).Name;
                    Int32.TryParse(stripped, out frame);
...
SpriteType GetSpriteType(string path)
{
    ...
    return SpriteType.Sprite; // it's probably a sprite anyway
}
...
Pressing ""No"" will cause the program to ignore these images.");
        ...
        // Check for duplicate filenames i don't care
